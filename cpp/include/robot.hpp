...
inline constexpr std::array<std::size_t, NUM_MOTORS + 1> MID_TO_IDX = {
    0,                   // [0]  unused                        OBS IDX
...
private:
     ...
     */
    void wait_mcu(McuClient& mcu, std::int32_t timeout_ms=10000);
     ...
     */
    std::unordered_map<std::string, std::vector<float>>& update_mcu_obs(const FxCliMap& mcu_data, McuClient& mcu);
     ...
     */
    bool is_limb_mid(std::size_t mid) {
        return std::find(WHEEL_MIDS.begin(), WHEEL_MIDS.end(), mid) == WHEEL_MIDS.end();
    }
    ...
    std::vector<float> last_action;     ///< Last commanded action (for monitoring)
    bool last_torque_ctrl;              ///< Last control mode used

    // Control gains
    std::vector<float> kp;              ///< Proportional gains
    std::vector<float> kd;              ///< Derivative gains
    bool gains_set;                     ///< Flag indicating if gains are configured

    // Motor configuration
    std::vector<std::string> motor_pattern;  ///< Motor connection patterns
    std::vector<std::string> motor_err;      ///< Motor error states

    // Battery monitoring
    std::string battery_voltage;        ///< Current battery voltage
    std::string battery_soc;            ///< Battery state of charge (%)

    // Connection health tracking
    int disconn_count;                  ///< Consecutive disconnection counter

    // Observation data
    std::unordered_map<std::string, std::vector<float>> obs;  ///< Current observation
